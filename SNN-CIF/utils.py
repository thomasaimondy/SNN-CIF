...
def clip_grad_norm_(params, max_norm, aggregate_norm_fn=None) -> torch.Tensor:
    ...
    # record the pretraining process
    """
    for number in range(len(params)):
        print(number)
        print(params[number].shape[0])
    print('end')
    """
    
    """
    params_0 = (params[200].detach().cpu().numpy())[0]  # array([0.], dtype=float32)
    params_0 = str(params_0) + '\n'
    params_0_write = open('/param_0.txt','a+')
    params_0_write.write(params_0)
    params_0_write.close()

    params_1 = (params[199].detach().cpu().numpy())[0]  # array([0.], dtype=float32)
    params_1 = str(params_1) + '\n'
    params_1_write = open('/param_1.txt','a+')
    params_1_write.write(params_1)
    params_1_write.close()

    params_2 = (params[198].detach().cpu().numpy())[0]  # array([0.], dtype=float32)
    params_2 = str(params_2) + '\n'
    params_2_write = open('/param_2.txt','a+')
    params_2_write.write(params_2)
    params_2_write.close()
    """
    ...
    if max_norm > 0:
        ...
        for g in grads + expert_grads:
            g.mul_(clip_coef)
        # grads[70] = grads[70] * 1000.0
        # grads[71] = grads[71] * 1000.0
    return total_norm
