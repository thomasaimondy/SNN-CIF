...
class CifTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if args.calulate_ctc_logits:
            ctc_proj = Linear(args.encoder_embed_dim, len(task.target_dictionary)).cuda()
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, target_lengths):
        ...
        if self.ctc_proj is not None:
            ctc_logits = self.ctc_proj(
                encoder_out["encoder_out"][0])  # T x B x V
        ...
        final_outputs = {
            ...
            "encoder_padding_mask":
                lengths_to_padding_mask(encoder_out["conv_lengths"][0]),    # B x T
            "ctc_logits":
                ctc_logits.transpose(0, 1),   # B x T x V

            # Cif module outputs
            "quantity_out": cif_out["quantity_out"],  # Quantity out for quantity loss calculation
    ...
    def get_ctc_output(self, src_tokens, src_lengths, **kwargs):
        ...
        encoder_outputs_padding_mask = \
            lengths_to_padding_mask(encoder_out["conv_lengths"][0])
...
class CifMiddleware(nn.Module):
    def __init__(self, args):
        ...
        if self.produce_weight_type == "dense":
            self.dense_proj = Linear(self.encoder_embed_dim, args.dense_cif_units_num).cuda()
    ...
    def forward(self, encoder_outputs, target_lengths=None, input_lengths=None):
        ...
        if len(encoder_outputs["encoder_padding_mask"]) != 0:
            ...
            assert input_lengths is not None, "Please ensure that input_lengths is provided."
        ...
        for i in range(max_length):
            ...
            cur_accumulated_weight = torch.where(
                cur_is_fired,
                cur_weight - remained_weight,  # pow()
                cur_weight + prev_accumulated_weight)  # [batch_size, 1]  # pow()
            ...
            cur_accumulated_state = torch.where(
                ...
                prev_accumulated_state + cur_weight * encoder_out[:, i, :])  # [batch_size, encoder_embed_dim]
            ...
            cur_fired_state = torch.where(
                ...
                torch.zeros([batch_size, encoder_embed_dim]).cuda())  # shape = [batch_size, encoder_embed_dim]
            ...
            accumulated_weights = torch.cat(
                (accumulated_weights, cur_accumulated_weight), 1)  # shape = [batch_size, Len]
        ...
        fired_marks = (torch.abs(fired_states).sum(-1) != 0.0).int()  # [batch_size, max_length]
        ...
        # print("cif inner state: ")
        # print(fired_marks.size())
        # print(fired_utt_length)
        ...
        return {
            ...
            "fired_states": fired_states,  # test1
            "weights": org_weight  # test2
        }


# 2nd-order dynamics
class CifMiddleware_1(nn.Module):
    def __init__(self, args):
        super().__init__()
        ...
        ### pretraining parameters including param_2, param_1, and param_0
        """
        self.param_2 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.param_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.param_0 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)

        self.param_2.data.fill_(0)
        self.param_1.data.fill_(0)
        self.param_0.data.fill_(0)
        """
    ...
    def forward(self, encoder_outputs, target_lengths=None, input_lengths=None):
        ...
        # if self.apply_scaling and target_lengths is not None:   # For validation debugging
            ...
            # (target_lengths + 1 because this target_lengths does not take <eos> into consideration)
            weight_sum = weight.sum(-1)  # weight_sum has shape [batch_size]
            normalize_scalar = torch.unsqueeze(target_lengths / weight_sum, -1)  # normalize_scalar has shape [batch_size, 1]
            # weight = weight * pow(normalize_scalar, 1 + 0.1 * torch.tanh(self.param_scalar))  # nonlinear 2
            weight = weight * normalize_scalar
        ...
        # fixed pretraining parameters
        param_2 = torch.tensor(-0.0991, dtype=torch.float32).cuda()     
        param_1 = torch.tensor(0.1413, dtype=torch.float32).cuda()   
        param_0 = torch.tensor(0.3231, dtype=torch.float32).cuda()
        ...
        for i in range(max_length):
            ...
            cur_accumulated_weight = torch.where(
                cur_is_fired,
                cur_weight - remained_weight,  # pow()
                0.2 * ((-0.2) + 2*torch.tanh(param_2)) * pow(prev_accumulated_weight, 2)  # 2-order  
                - 0.2 * ((-0.2) + 1*torch.tanh(param_1)) * prev_accumulated_weight        # 1-order
                + (1 - 0.3 * ((-0.2) + 1*torch.tanh(param_0))) * cur_weight + prev_accumulated_weight)
...
# 1st-order dynamics using dynamic threshold
class CifMiddleware_2(nn.Module):
    def __init__(self, args):
        super().__init__()
    ...
    def forward(self, encoder_outputs, target_lengths=None, input_lengths=None):
        ...
        # if self.apply_scaling and target_lengths is not None:   # For validation debugging
            ...
            weight_sum = weight.sum(-1)  # weight_sum has shape [batch_size]
            normalize_scalar = torch.unsqueeze(target_lengths / weight_sum, -1)  # normalize_scalar has shape [batch_size, 1]
            # weight = weight * pow(normalize_scalar, 1 + 0.1 * torch.tanh(self.param_scalar))  # 非线性2
            weight = weight * normalize_scalar
        ...
        adaptive_threshold = self.cif_threshold * torch.ones(batch_size).cuda()  # 初始值0.99
        ...
        for i in range(max_length):
            ...
            # Decide whether positioning a boundary
            # fire_test = (pre_v - torch.ones([batch_test, dim_test]).cuda() * threshold_test) >= 0
            cur_is_fired = ((prev_accumulated_weight + weight[:, i]) >= adaptive_threshold).unsqueeze(-1)  # self.cif_threshold
            # print(cur_is_fired.squeeze(-1))
            # cur_is_fired with shape [batch_size, 1]
            
            # dynamic threshold
            adaptive_threshold = torch.where(  
                cur_is_fired.squeeze(-1),
                adaptive_threshold * 0.95 + 0.1,  # 
                adaptive_threshold * 0.95)  # 0.95
            ...
            #     0.2 * pow(prev_accumulated_weight, 2) + 0.2 * prev_accumulated_weight + 0.6 * cur_weight + prev_accumulated_weight)
...
# double-neuron dynamics
class CifMiddleware_3(nn.Module):
    def __init__(self, args):
        super().__init__()
    ...
    def forward(self, encoder_outputs, target_lengths=None, input_lengths=None):
        ...
        step_flag = 3 * torch.ones(batch_size).unsqueeze(dim=-1).cuda()

        accumulated_U = torch.zeros(batch_size, 0).cuda()
        ...
        for i in range(max_length):
            ...
            # Get previous states from the recorded tensor
            prev_accumulated_weight = torch.zeros([batch_size]).cuda() if i == 0 else accumulated_weights[:, i - 1]
            prev_accumulated_U = (-0.99)*torch.ones(batch_size).cuda() if i == 0 else accumulated_U[:, i - 1]  # Un-1
            # U0 = -0.99
            prev_accumulated_state = \
                torch.zeros([batch_size, encoder_embed_dim]).cuda() if i == 0 else accumulated_states[:, i - 1, :]

            # Decide whether positioning a boundary
            cur_is_fired = ((prev_accumulated_weight + weight[:, i] - 0.05 * prev_accumulated_U) >= self.cif_threshold).unsqueeze(dim=-1)
            # cur_is_fired with shape [batch_size, 1]
            ...
            # cur_weight has shape [batch_size, 1]
            prev_accumulated_weight = torch.unsqueeze(prev_accumulated_weight, -1)
            prev_accumulated_U = torch.unsqueeze(prev_accumulated_U, -1)
            # prev_accumulated_weight also has shape [batch_size ,1]
            remained_weight = torch.ones_like(prev_accumulated_weight).cuda() - prev_accumulated_weight
            # remained_weight with shape [batch_size ,1]
            ...
            cur_accumulated_weight = torch.where(
                cur_is_fired,
                cur_weight - 0.05 * prev_accumulated_U - remained_weight,  # pow()
                cur_weight - 0.05 * prev_accumulated_U + prev_accumulated_weight)  # [batch_size, 1]  # pow()

            step_flag = torch.where(
                cur_is_fired,
                1 * torch.ones(batch_size).unsqueeze(dim=-1).cuda(),
                torch.where(
                    step_flag == 1,
                    2 * torch.ones(batch_size).unsqueeze(dim=-1).cuda(),
                    torch.where(
                        step_flag == 2,
                        3 * torch.ones(batch_size).unsqueeze(dim=-1).cuda(),
                        step_flag
                    )
                )
            )

            cur_accumulated_U = torch.where(
                step_flag == 1, 
                (-0.05 * prev_accumulated_U + prev_accumulated_U) / 0.95 + 2,
                torch.where(
                    step_flag == 2,
                    -0.05 * prev_accumulated_U + prev_accumulated_U - 2.1,
                    -0.05 * prev_accumulated_U + prev_accumulated_U
                    )
            )
            ...
            accumulated_weights = torch.cat(
                (accumulated_weights, cur_accumulated_weight), 1)  # shape = [batch_size, Len]
            accumulated_U = torch.cat(
                (accumulated_U, cur_accumulated_U), 1)  # shape = [batch_size, Len]
            accumulated_states = torch.cat(
                (accumulated_states, torch.unsqueeze(cur_accumulated_state, 1)), 1)  # shape = [B, L, D]
            fired_states = torch.cat(
                (fired_states, torch.unsqueeze(cur_fired_state, 1)), 1)  # shape = [B, L, D]
...
class CifProjDecoder(FairseqDecoder):
    ...
    def forward(self, prev_output_tokens=None, cif_out=None, **kwargs):
        x = cif_out["ctxt_cif_out"] \
            if cif_out["ctxt_cif_out"] is not None else cif_out["cif_out"]
...
class CifNarTransformerDecoder(CifProjDecoder):
    def __init__(self, cfg, dictionary):
        ...
        assert self.decoder_embed_dim == self.pre_final_proj_dim, \
    ...
    def forward(self, prev_output_tokens=None, cif_out=None, **kwargs):
        x = cif_out["ctxt_cif_out"] \
            if cif_out["ctxt_cif_out"] is not None else cif_out["cif_out"]  # B x T x C
...
def base_architecture(args):
    ...
    args.encoder_layers = getattr(args, "encoder_layers", 12)  # 12
    ...
    args.decoder_layers = getattr(args, "decoder_layers", 4)  # 4
    ...
    args.conv_cif_output_channels_num = getattr(args, "conv_cif_output_channels_num", 768)
    ...
    args.tail_handling_firing_threshold = getattr(args, "tail_handling_firing_threshold", 0.5)
    ...
    args.pre_final_proj_dim = getattr(args, "pre_final_proj_dim", args.decoder_embed_dim)
    ...
    args.decoder_attention_dropout = getattr(args, "decoder_attention_dropout", args.attention_dropout)
    args.decoder_activation_dropout = getattr(args, "decoder_activation_dropout", args.activation_dropout)

    # Other settings
    args.calulate_ctc_logits = getattr(args, "calulate_ctc_logits", True)


@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_alpha")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_1")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
    ...
    args.conv_cif_layer_num = getattr(args, "conv_cif_layer_num", 1)
    args.conv_cif_width = getattr(args, "conv_cif_width", 3)
    args.conv_cif_output_channels_num = getattr(args, "conv_cif_output_channels_num", 512)
    args.conv_cif_dropout = getattr(args, "conv_cif_dropout", args.dropout)
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_2")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_3")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_4")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_5")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_6")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_7")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_8")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp1_9")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
...
@register_model_architecture(model_name="cif_transformer", arch_name="cif_transformer_exp6")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)
    # Convolutional subsampler
    args.conv_kernel_sizes = getattr(args, "conv_kernel_sizes", "5,5")
